...
var selectAllStatement = "SELECT * FROM presupuestos ORDER BY nombre";
    ...
    db.transaction(function(tx) {
        tx.executeSql(selectAllStatement, [], function(tx, result) {
            ...
            for (var i = 0, item = null; i < dataset.length;  i++) {
                item = dataset.item(i);
                $('#listado').append('<li><a href=#>'+ item['nombre'] 
                    + ' &lt;' + item['email'] + '&gt; ('
                    + item['precio'].toFixed(2) + ')</a></li>');
...
$(document).ready(function () {
	//dropTable();
    initDatabase();
    //insertRecord("dave", "dave.padrino@jajaja.jojo", 123123123);
});